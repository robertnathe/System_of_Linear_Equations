...
int Jacobi_Residual(const matrix<double>& A, vector<double>& B, vector<double>& X, unsigned int num_rows, unsigned int num_cols, double tolerance, unsigned int Max_Iter) {
...
int Gauss_Seidel_Residual(const matrix<double>& A, vector<double>& B, vector<double>& X, unsigned int num_rows, unsigned int num_cols, double tolerance, unsigned int Max_Iter) {
  // Initialize iteration counter and residual
  int iter = 0;
  double residual = 1.0;
  ...
  // Loop until convergence or maximum iterations reached
  while (iter < Max_Iter && residual > tolerance) {
    // Update each element of X using Gauss-Seidel iteration
    residual = 0.0;
    for (int i = 0; i < num_rows; ++i) {
      double new_x = 0.0;
      for (int j = 0; j < num_cols; ++j) {
...
int SOR_Residual(const matrix<double>& A, vector<double>& B, vector<double>& X, unsigned int num_rows, unsigned int num_cols, double omega, double tolerance, unsigned int Max_Iter) {
  // Initialize iteration counter and residual
  int iter = 0;
  double residual = 1.0;
  ...
  // Loop until convergence or maximum iterations reached
  while (iter < Max_Iter && residual > tolerance) {
    // Update each element of X using SOR iteration
    residual = 0.0;
    #pragma omp parallel for reduction(+:residual)
    for (int i = 0; i < num_rows; ++i) {
      double new_x = 0.0;
      ...
      new_x = omega * new_x / A[i][i] + (1.0 - omega) * X[i];
      residual += std::pow(X[i] - new_x, 2.0);
      X[i] = new_x;
    }
    iter++;
