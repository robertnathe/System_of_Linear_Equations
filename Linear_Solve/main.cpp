...
int ST_Input(const matrix<double>& A, double B[], double X[], int num_rows, int num_cols);
int Tridiagonal_Input(double subdiagonal[], double diagonal[], double superdiagonal[], const matrix<double>& A, double B[], double X[], int num_rows, int num_cols);
...
// Begin code here.
  ...
  // Change from num_cols-1 to num_cols
...
// End code here.
  Tridiagonal_Input (&subdiagonal[0], &diagonal[0], &superdiagonal[0], A, &B[0], &X[0], num_rows, num_cols);
  delete [] subdiagonal;
  delete [] diagonal;
...
//         signalling an error.
  for (i = 0; i < num_cols; i++) if (diagonal[i] == 0.0) err = -1;
  //forward elimination
  ...
  X[num_cols-1]=B[num_cols]/diagonal[num_cols];
  ...
  print_tridiagonal_results(A, &B[0], &X[0], num_rows, num_cols);
  return 0;
}
...
int main ()
{
   ...
   // Begin TDMA
   ST_Input(A, &B[0], &X[0], num_rows, num_cols);   
   // End TDMA
